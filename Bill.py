...
class Bill():
    """
    Bill aka The Snake Charmer is the ruthless leader of the Snake
    Assassination Squad — the one who calls the shots and charms serpents into
    submission.

    A true orchestrator, he commands the training, testing, and exhibition of
    the AI snake with deadly precision. Whether you're watching a flawless
    exhibition or throwing your agent into the pit to learn from failure,
    Bill is behind the scenes, cold and calculated.
    This isn't just an interface — it's a Kill Bill kind of control.
    """
    ...
    def train(self):
        ...
        for i in range(self.sessions):
            ...
            epid = f"{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}\
    ...
    def train_log(self):
        ...
        agente = Agent(
            ...
            gamma=gamma,
            nn_learningrate=nn_learningrate)
        for input_data in Interpreter.load_log_file_chunks(
                self.logfile, chunk_size=chunk_size, skip=0):
            i = i + 1
