...
def horizontal_reflection(df):
    ...
    df = df.with_columns(
        pl.col("action").replace({"0": "1", "1": "0"}).alias("action")
    )
    return df
...
class Interpreter():
    ...
    def build_inputs_from_views(df):
        ...
        df = df.with_columns([
            pl.when(pl.col(f"{direction}_{char}_input") >= 0).then(
                1/(1+pl.col(f"{direction}_{char}_input"))).otherwise(-1).alias(f"{direction}_{char}_inv_input")
            for direction in ["north_view", "south_view", "east_view", "west_view"]
            for char in ["W", "S", "R", "G"]
        ])
        ...
        df = df.with_columns([
            ((pl.col(f"{direction}_S_input") == 0) | (pl.col(f"{direction}_W_input") == 0)).cast(
    ...
    def print_view(view: pl.DataFrame):
        ...
        assert all(col in view.columns for col in [
                   "north_view", "south_view", "east_view", "west_view"]), "Make sure to pass a view to this"
    ...
    def load_single_log(jdict) -> pl.DataFrame:
        ...
        inputs = [Interpreter.build_inputs_from_views(
            Interpreter.extract_views_from_board(np.array(x))) for x in jdict["states"]]
